...
import { View, Text, StyleSheet, StatusBar, Image, ScrollView, SafeAreaView, TouchableOpacity, ToastAndroid, Switch } from 'react-native';
...
import { signUpDefault } from '../signUp/actions/signUp.actions';
...
function Login({ navigation }) {
  ...
  const [fleetOwner, setFleetOwner] = React.useState(false);
  ...
  const handleLogin = () => {
    ...
    let data = {email, password, fleetOwner}
    dispatch(logIn(data));
  }
  ...
  const handleFleetOwner = () => {
    setFleetOwner(previousState => !previousState);
    // console.log('fleetOwned' + fleetOwned)
  };
      ...
      <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps={'handled'} >
        ...
        <View style={styles.formContainer}>
          ...
          <View style={styles.switchContainer}>
            <Switch
              trackColor={{ false: "#767577", true: colors.mainColor+'70' }}
              thumbColor={fleetOwner ? colors.mainColor : "#f4f3f4"}
              ios_backgroundColor="#3e3e3e"
              onValueChange={handleFleetOwner}
              value={fleetOwner}
            />
            <Text style={styles.switchInfo}>Login as fleet owner</Text>
          </View>
          ...
          <View style={styles.qusContainer}>
            <QusOpt question={'New user'} option={'Sign Up for a new account'} onPress={() => navigation.navigate('RiderSignUp')} />
          </View>
        </View>
      </ScrollView>
    </View>
