...
import { changePassword, logOut, updateProfile } from '../login/actions/login.actions';
import { validateEmail, validateMobile, validateName, validatePassword } from '../../services/validation.service';
import { resetState } from '../home/actions/home.action';
...
function Settings({ navigation }) {
  ...
  React.useEffect(() => {
    setName(user.fleetOwner ? user.companyName : user.fullName);
    let mobile = user.mobile.value;
    setMobile(`0${mobile.substring(4)}`);
    setEmail(user.email.value);
    if (user.profilePicture) {
      setProfilePics(user.profilePicture.value);
      setMimeType(user.profilePicture.mimeType);
    }
  }, [])
  ...
  const handleProfileUpdate = () => {
    ...
    let userData = {
      ...
      fleetOwner: user.fleetOwner,
      details: {
        ...(user.fleetOwner) && {fleetId: user.id, companyName: name},
        ...(!user.fleetOwner) && {riderId: user.id, fullName: name},
  ...
  const handleLogOut = () => {
    ...
    deleteFromStorage('driverDestination')
    deleteFromStorage('notification')
    deleteFromStorage('driverOnline')
      .then((response) => {
        dispatch(logOut())
        dispatch(resetState())
      })
    ...
    <View style={styles.main}>
      ...
      <View style={styles.userInfo}>
        ...
        <View style={styles.userData}>
          <Text style={styles.userName}>{user.fullName ? user.fullName : user.companyName}</Text>
          <Text style={styles.userEmail}>{user.email.value}</Text>
          <Text style={styles.userNum}>{user.mobile.value}</Text>
        </View>
      ...
      {!user.fleetOwner && (
        <View style={styles.option}>
          <Text style={styles.optionTitle}>Active Bike</Text>
          {/* <Ionicons name={'chevron-down'} color={colors.text.white} size={hp(18)} /> */}
        </View>
      )}
      <View style={styles.bikeInfo}>
        <Text style={styles.bikeName}>{user.bikeManufacturer} {user.bikeType}</Text>
        <Text style={styles.bikeNum}>{user.plateNumber}</Text>
      </View>
      {/* <View style={styles.option}>
        <Text style={styles.optionTitle}>Driver's Portal</Text>
      ...
      <TouchableNativeFeedback onPress={() => handleNavigation('Earning')}>
        <View style={styles.option}>
          <Text style={styles.optionTitle}>Earnings</Text>
          <Ionicons name={'chevron-forward'} color={colors.text.white} size={hp(18)} />
        </View>
      ...
      <TouchableNativeFeedback onPress={() => navigation.navigate('Support')}>
        <View style={styles.option}>
