...
function StepFour({ onPressNext, errorRiderCardDetails, errorRiderCards }) {
  ...
  const handleFileUpload = async (type) => {
    await ImagePicker.launchImageLibraryAsync({
      ...
      .then((response) => {
        ...
        switch (type) {
          case 'riderCard':
            signUpValues = {...signUpValues, ridersCardValue: response.base64, ridersCardMimeType: mimeType}
      ...
      <View style={styles.form}>
        ...
        <View style={styles.cardDetailsContainer}>
          ...
          <View style={styles.cardDetailsContent}>
            ...
            <RegSelector text={state.userData.riderCardDay ? state.userData.riderCardDay : 'Day'} marginTop={hp(4)} onPress={() => handleDate('day')} />
            <RegSelector text={state.userData.riderCardMonth ? state.userData.riderCardMonth : 'Month'} marginTop={hp(8)} onPress={() => handleDate('month')} />
            <RegSelector text={state.userData.riderCardYear ? state.userData.riderCardYear : 'Year'} marginTop={hp(8)} onPress={() => handleDate('year')} />
          </View>
          <View style={styles.btnContainer}>
            {state.userData.ridersCardValue && (
              <Ionicons name={'checkmark-circle'} size={hp(18)} color={colors.green} />
            )}
            <Button
              onPress={() => handleFileUpload('riderCard')}
              width={wp(103)} height={32} title={'+ Upload file'}
        ...
        <View style={{marginTop: hp(26)}}>
            <Button width={wp(103)} height={32} title={'Finish adding bike'} fontSize={hp(10)} fontWeight={'700'} borderRadius={wp(10)} onPress={onPressNext} />
          </View>
      </View>
    </View>
  );
