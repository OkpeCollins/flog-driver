...
function PayOptCard({ type, title, marginTop, option = false, optColor, onPress, price, titleSize = wp(14), width = wp(311), paymentStatus, disabled}) {
  // let Component;
  // switch (type) {
  //   case 'visa':
  //       componet = Visa;
    ...
    <TouchableOpacity style={{ marginTop }} onPress={onPress} disabled={disabled}>
      <View style={[styles.main, {width}]}>
        ...
        {type === 'VISA' ? (
            ...
            <Cash />
          ) : type === 'MASTERCARD' ? <Mastercard /> : type === 'VERVE' ? <Verve /> : type === 'Card' ? <Mastercard /> : (<Ionicons name={'add-circle'} size={hp(20)} color={colors.blue} />)}
        </View>
